...
impl Monomorphizer {
    ...
    pub fn register_generic_type(&mut self, name: String, type_params: Vec<String>, fields: Vec<Field>) {
    ...
    pub fn generate_monomorphic_struct(&mut self, monomorph: &MonomorphicType, type_to_c: impl Fn(&Type) -> String) -> String {
        ...
        if let Some((type_params, fields)) = self.generic_types.get(&monomorph.base_name) {
            ...
            for field in fields {
                let concrete_type = substitute_type_params(&field.field_type, type_params, &monomorph.type_args);
...
pub fn substitute_type_params(generic_type: &Type, type_params: &[String], concrete_types: &[String]) -> Type {
    match generic_type {
        Type::Custom(name) => {
