...
impl<'a> StatementCompiler<'a> {
    ...
    pub fn compile_var_decl(&mut self, name: &str, var_type: &Option<Type>, value: &Expression) {
        ...
        if let Some(t) = var_type {
            ...
            self.variables
                .insert(name.to_string(), inferred_type.to_string());
    ...
    pub fn compile_statement_with_indent(&mut self, statement: &Statement, base_indent: &str) {
        ...
        match statement {
            ...
            Statement::ValDecl {
                ...
                self.compile_statement_with_indent(
                    &Statement::VarDecl {
                        name: name.clone(),
            ...
            Statement::Expression(expr) => match expr {
                Expression::NamespacedFunctionCall {
                    ...
                    if namespace == "stdio" && function == "print" && args.len() == 1 {
                        ...
                        self.main_code
                            .push_str(&format!("{}{}", indent.trim_end(), print_stmt));
                    }
                }
                _ => {
                    let expr_compiler = ExpressionCompiler::new(self.variables);
            ...
            _ => {
                ...
                self.main_code
                    .push_str(&format!("{}/* Unhandled statement type */;\n", indent));
            }
        }
    }
}
