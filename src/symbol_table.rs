...
impl SymbolTable {
    ...
    pub fn declare_variable(
        ...
        if let Some(scope) = self.get_scope_mut(self.current_scope) {
            scope.add_symbol(name.clone(), symbol_id).map_err(|e| {
                CompileError::ParseError(ParseError::InvalidSyntax {
                    message: e,
                    location,
                })
            })?;
    ...
    pub fn get_unused_symbols(&self) -> Vec<SymbolId> {
        self.symbols
            ...
            .enumerate()
            .filter(|(_, symbol)| {
                !symbol.used && !matches!(symbol.kind, SymbolKind::Function { .. })
            })
            .map(|(id, _)| id)
            .collect()
    }
...
mod tests {
    ...
    fn test_scope_management() {
        ...
        let func_scope = symbol_table.enter_scope(ScopeKind::Function {
            name: "test".to_string(),
        });
    ...
    fn test_variable_declaration() {
        ...
        let symbol_id = symbol_table
            .declare_variable("x".to_string(), Type::Integer, true, None)
            .unwrap();
    ...
    fn test_scope_shadowing() {
        ...
        let global_var = symbol_table
            .declare_variable("x".to_string(), Type::Integer, false, None)
            .unwrap();

        // Enter function scope
        symbol_table.enter_scope(ScopeKind::Function {
            name: "test".to_string(),
        });

        // Declare variable with same name in function scope
        let local_var = symbol_table
            .declare_variable("x".to_string(), Type::String, true, None)
            .unwrap();
    ...
    fn test_redeclaration_error() {
        ...
        let result = symbol_table.declare_variable("x".to_string(), Type::String, false, None);
    ...
    fn test_legacy_variables_conversion() {
        ...
        symbol_table
            ...
            .unwrap();
        symbol_table
            .declare_variable("name".to_string(), Type::String, false, None)
            .unwrap();
        symbol_table
            .declare_variable("flag".to_string(), Type::Bool, true, None)
            .unwrap();
    ...
    fn test_unused_symbols() {
        ...
        let used_var = symbol_table
            .declare_variable("used".to_string(), Type::Integer, true, None)
            .unwrap();
        let unused_var = symbol_table
            .declare_variable("unused".to_string(), Type::String, false, None)
            .unwrap();
