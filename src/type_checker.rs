use crate::ast::{BinaryOperator, Expression, Program, Statement, Type, UnaryOperator};
        ...
        match statement {
            ...
            Statement::NativeBlock { functions, .. } => {
                for native_func in functions {
                    let param_types: Vec<(String, Type)> = native_func
                        .params
                        .iter()
                        .map(|p| (p.name.clone(), p.param_type.clone()))
                        .collect();
            ...
            Statement::ExternBlock { functions, .. } => {
                for extern_func in functions {
                    let param_types: Vec<(String, Type)> = extern_func
                        .params
                        .iter()
                        .map(|p| (p.name.clone(), p.param_type.clone()))
                        .collect();
        ...
        match statement {
            ...
            Statement::ForIn {
                ...
                match iterable_type {
                    ...
                    _ => {
                        self.add_error(
                            format!("Cannot iterate over {:?}, expected Array", iterable_type),
                            0,
                            0,
                        );
                    }
        ...
        match expression {
            ...
            Expression::ArrayAccess { array, index } => {
                ...
                match array_type {
                    ...
                    _ => {
                        self.add_error(
                            format!("Cannot index into {:?}, expected Array", array_type),
                            0,
                            0,
                        );
            ...
            Expression::Dereference { operand } => {
                ...
                match expr_type {
                    ...
                    _ => {
                        self.add_error(
                            format!("Cannot dereference {:?}, expected Pointer", expr_type),
                            0,
                            0,
                        );
                        Type::String // Default type
    ...
    fn check_binary_operation(
        ...
        match operator {
            Add | Subtract | Multiply | Divide | Modulo => {
                if self.types_compatible(&Type::Integer, left)
                    && self.types_compatible(&Type::Integer, right)
                {
                    Type::Integer
                } else {
                    self.add_error(
            ...
            Equal | NotEqual => {
                if !self.types_compatible(left, right) {
                    self.add_error(
                        format!("Cannot compare different types: {:?} and {:?}", left, right),
                        0,
                        0,
                    );
                }
